...
interface PagoRepository : CrudRepository<Pago, Long>{
    ...
    @Query(value = "select p from Pago p where p.evento.empresa.id = ?1 AND p.fechaBaja IS NULL")
    fun findAll(id: Long,  pageable : Pageable) : Page<Pago>

    @Query(value = "SELECT p FROM Pago p WHERE p.evento.empresa.id = ?1 AND (p.evento.codigo ILIKE %?2% OR p.evento.cliente.nombre ILIKE %?2%)")
